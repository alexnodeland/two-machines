...
([D-028](00-decision-log.md#d-028--jsdom-cannot-test-web-audio-three-tier-strategy-instead--firm))
...
([D-027](00-decision-log.md#d-027--engines-are-split-pure-from-wiring--firm)).

Table-driven, with the worked values in
[04 §3](04-cycles-engine-spec.md#worked-values-for-test-fixtures) as fixtures. Property
...
[03 §8](03-audio-engine-spec.md#8-behavioural-contract) against the oracle:
...
- **feedback 1.18 → output never exceeds the limiter ceiling** (a safety test, see
  [06 §2](06-accessibility-and-interaction.md#2-audio-consent))
...
- It does **not** mean the prose is accurate. That is what
  [12 · References](12-references.md) is for.
...
| **`quiver.worklet.js` and `quiver_bg.wasm` resolve at the deployed prefix** | [D-037](00-decision-log.md#d-037--worklet-and-wasm-urls-must-be-prefix-aware--firm) — invisible until someone presses play |
| No raw internal `href` 404s under the prefix | [D-029](00-decision-log.md#d-029--github-pages-project-page-therefore-pathprefix-is-required--firm) |
| Audio does not start without a gesture | [06 §2](06-accessibility-and-interaction.md#2-audio-consent) |
| Every instrument works with the mic denied | floor 1 |
| `axe-core` clean on every page | [06 §8](06-accessibility-and-interaction.md#8-testing) |
| Keyboard-only path through both L's | floor 5 |
| Reduced-motion emulation | [06 §3](06-accessibility-and-interaction.md#3-motion) |
| Every citation link resolves | [12](12-references.md) |
...
- Fixtures for the cycle math come from [04 §3](04-cycles-engine-spec.md), which is
