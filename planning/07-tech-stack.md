...
| Layer | Choice | Notes |
|---|---|---|
| Framework | **Gatsby 5** | Client decision over the brief's Astro suggestion ([D-024](00-decision-log.md#d-024--gatsby-5--typescript-strict--firm)) |
| Language | **TypeScript, `strict: true`** | No DSP in TypeScript — that is quiver's job |
| Package manager | **Bun** | Front door only; Gatsby runs on Node ([D-025](00-decision-log.md#d-025--bun-is-the-package-manager-and-script-runner-node-is-the-runtime--firm)) |
...
| DSP | **quiver via `@quiver-dsp/wasm`** | AudioWorklet ([D-035](00-decision-log.md#d-035--the-rig-is-a-quiver-patch-running-in-an-audioworklet--firm)) |
| Tests | **Vitest** + Testing Library + Playwright | [08](08-testing-strategy.md) |
| Task runner | **`justfile`** | Matches the reference repo |
| Hosting | **GitHub Pages**, project page | [09](09-ci-cd.md) |
...
([D-027](00-decision-log.md#d-027--engines-are-split-pure-from-wiring--firm)); if it
...
- `pathPrefix` is required — project page, not custom domain
  ([D-029](00-decision-log.md#d-029--github-pages-project-page-therefore-pathprefix-is-required--firm)).
...
`withPrefix` ([D-037](00-decision-log.md#d-037--worklet-and-wasm-urls-must-be-prefix-aware--firm)):
...
[06](06-accessibility-and-interaction.md), no chapter depends on audio to make its point,
...
- **`no-restricted-imports`** preventing `src/audio/math/**` from importing anything
  outside itself — protects [D-027](00-decision-log.md#d-027--engines-are-split-pure-from-wiring--firm).
- **A lint rule or test banning raw internal `href`s**, forcing `Link`/`withPrefix` —
  protects [D-029](00-decision-log.md#d-029--github-pages-project-page-therefore-pathprefix-is-required--firm).
